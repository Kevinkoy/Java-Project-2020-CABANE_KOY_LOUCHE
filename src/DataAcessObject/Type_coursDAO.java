...
public class Type_coursDAO extends DAO<Type_cours> {
    ...
    public boolean create(Type_cours obj) {
        try {
            ...
            String sql = "INSERT INTO `type_cours`(`ID`, `Nom`) VALUES(" + "NULL" + ",'" + obj.getNom() + "');";
            // PrepareStatement
    ...
    public boolean delete(Type_cours obj) {
        ...
        Type_cours copie = obj;
        try {
            // REQUETE SQL : DELETE
            String sql = "DELETE FROM `type_cours` WHERE ID ='" + obj.getId() + "' AND Nom ='" + obj.getNom() + "'";
            // PrepareStatement
            PreparedStatement preparedstatement = this.connection.prepareStatement(sql);
            // ResultSet (result)
            int result = preparedstatement.executeUpdate(sql);
    ...
    public boolean update(Type_cours obj) {
        try {
            // REQUETE SQL : UPDATE
            String sql = "UPDATE `type_cours` SET `ID`='" + obj.getId() + "',`Nom`='" + obj.getNom() + "' WHERE ID='" + obj.getId() + "'";
            // PrepareStatement
            PreparedStatement preparedstatement = this.connection.prepareStatement(sql);
            // ResultSet (result)
            int result = preparedstatement.executeUpdate(sql);
    ...
    public Type_cours find(int id) {
        ...
        Type_cours returned = new Type_cours();
        try {
            // REQUETE
            String sql = "SELECT * FROM `type_cours` WHERE ID ='" + id + "'";
            // PrepareStatement
            ...
            if (result.next()) {
                /// return = RESULTAT
                returned = new Type_cours(result.getInt("ID"), result.getString("Nom"));
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            //e.printStackTrace();
    ...
    public Type_cours find(String nom) {
        ...
        try {
            // REQUETE
            String sql = "SELECT * FROM `type_cours` WHERE Nom ='" + nom + "'";
            // PrepareStatement
            PreparedStatement preparestatement = this.connection.prepareStatement(sql);
            // ResultSet
            ResultSet result = preparestatement.executeQuery(sql);
    ...
    public Type_cours find(Cours obj) {
        /// CONSTRUCTEUR NULL (PAR DEFAUT)
        Type_cours returned = new Type_cours();
        try {
            // REQUETE
            String sql = "SELECT * FROM `type_cours` WHERE ID ='" + obj.getId() + "' AND Nom='" + obj.getNom() + "'";
            // PrepareStatement
            PreparedStatement preparestatement = this.connection.prepareStatement(sql);
            // ResultSet
            ResultSet result = preparestatement.executeQuery(sql);
